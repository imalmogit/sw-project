...
# --- 2. counters -----------------------------------------------------------
say "[2/5] perf stat, $REPS repetitions"
...
perf stat -r "$REPS" \
    -e task-clock,context-switches,page-faults,cycles,instructions,cache-references,cache-misses,branches,branch-misses \
    python -m pyperformance run -b "$BENCH" \
    > "$RES/perf_stat_stdout.txt" 2> "$RES/perf_stat.txt"
tail -25 "$RES/perf_stat.txt"

# --- 3. sampling profile ---------------------------------------------------
say "[3/5] perf record (the specification's documented form)"
perf record -F 999 -g -o "$RES/perf.data" -- \
    python -m pyperformance run -b "$BENCH" \
    > "$RES/perf_record.log" 2>&1 \
    || echo "  (perf record returned non-zero -- check $RES/perf_record.log)"
[ -s "$RES/perf.data" ] && echo "  perf.data : $(du -h "$RES/perf.data" | cut -f1)" \
                        || die "no perf.data produced"
