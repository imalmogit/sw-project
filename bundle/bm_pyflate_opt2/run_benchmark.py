...
class HuffmanTable(object):
    ...
    def build_lookup(self):
        """Group the table by code length into dicts keyed on the code word.

        The original scanned every entry linearly, touching three Python
        attributes per candidate. self.table is already sorted by (bits,
        code), so grouping preserves the ascending-length search order and
        replaces the scan with one dict lookup per distinct code length.
        """
        fwd = []
        rev = []
        for x in self.table:
            if not fwd or fwd[-1][0] != x.bits:
                fwd.append((x.bits, {}))
                rev.append((x.bits, {}))
            fwd[-1][1].setdefault(x.symbol, x.code)
            rev[-1][1].setdefault(x.reverse_symbol, x.code)
        self._lookup_fwd = fwd
        self._lookup_rev = rev

    def find_next_symbol(self, field, reversed=True):
        groups = self._lookup_rev if reversed else self._lookup_fwd
        snoopbits = field.snoopbits
        for bits, table in groups:
            code = table.get(snoopbits(bits))
            if code is not None:
                field.readbits(bits)
                return code
...
def move_to_front(l, c):
    # Original rebuilt the whole list every call:
    #     l[:] = l[c:c + 1] + l[0:c] + l[c + 1:]
    # which allocates three slices and two concatenations per decoded
    # symbol. pop/insert moves the same elements with two memmoves and
    # no allocation.
    l.insert(0, l.pop(c))
...
if __name__ == '__main__':
    ...
    runner.bench_time_func('pyflate_opt2', bench_pyflake, filename)
